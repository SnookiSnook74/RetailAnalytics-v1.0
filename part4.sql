...
--Основная логика:
-- Функция активируется, если chosen_method = 2.
-- Сначала определяется максимально допустимая глубина скидки на основе истории покупок и заданной доли маржи (с помощью подзапроса с использованием WITH).
-- Далее, для каждого клиента рассчитывается средний чек за заданное количество последних транзакций и максимальная возможная скидка, учитывая его историю покупок и другие параметры.
-- Скидки округляются с шагом в 5%.
-- Функция возвращает идентификатор клиента, рассчитанный средний чек, название группы товаров, а также рекомендуемую глубину скидки.
        ...
        from
            (
                ...
                from
                    (
                        select
                            customercards.customer_id as customer_id_inner,
                            transactions.transaction_summ as transaction_summ_inner,
                            row_number() over(
                                partition by customercards.customer_id
                                order by
                                    transactions.transaction_datetime
                            ) as transaction_summ_inner_counter,  -- переменная для учета количества транзакций, начиная счет с последних
                            sku_groups.group_name as group_name_inner,
                            groupsview.group_affinity_index as group_affinity_index_inner,
                            groupsview.group_minimum_discount as group_minimum_discount_inner,
                            pre_offer_discount_depth
                        from
                            transactions
                            inner join customercards on customercards.customer_card_id = transactions.customer_card_id
                            inner join groupsview on groupsview.customer_id = customercards.customer_id
                            inner join sku_groups on sku_groups.group_id = groupsview.group_id
                            inner join purchase_history_total on purchase_history_total.customer_id = customercards.customer_id
                            and purchase_history_total.group_id = groupsview.group_id
                        where
                            groupsview.group_discount_share < maximum_share / 100
                            and groupsview.group_churn_rate <= maximum_churn_index
                    ) AS tmp_from1
                where
...
SELECT * FROM generate_personal_offer(
    2, 100, 1.15, 3, 70, 30);