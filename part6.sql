-- DROP FUNCTION IF EXISTS generate_cross_selling;
-- Функция анализирует группы товаров, с которыми клиент редко взаимодействует (на основе индекса оттока и индекса стабильности потребления).
-- Для каждой группы товаров выбираются SKU с максимальной маржой.
-- Из этих SKU выбираются те, доля которых в своей группе не превышает заданной доли (maximum_SKU_share).
-- Функция также учитывает, что рассматриваются покупки только в основных магазинах клиентов.
-- На основе этих данных функция рассчитывает предложенную скидку для каждого SKU.
